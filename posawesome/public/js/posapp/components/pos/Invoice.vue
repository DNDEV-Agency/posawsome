<template>
  <div>
    ...
    <v-card style="max-height: 70vh;" class="cards my-0 py-0 mt-3 grey lighten-5">
      ...
      <v-row align="center" class="items px-2 py-1 mt-0 pt-0" v-if="pos_profile.posa_use_delivery_charges">
        ...
        <v-col cols="4" class="pb-0 mb-0 pt-0">
          <v-text-field dense outlined color="primary" :label="frappe._('Delivery Charges Rate')" background-color="white"
            hide-details :value="formtCurrency(delivery_charges_rate)" :prefix="currencySymbol(pos_profile.currency)"
    ...
    "></v-text-field>
                  ...
                  <v-col cols="4">
                    <v-text-field dense outlined color="primary" :label="frappe._('Available QTY')"
                      background-color="white" hide-details :value="formtFloat(item.actual_qty)" disabled></v-text-field>
                  </v-col>
                  <v-col cols="4">
    ...
    <v-card class="cards mb-0 mt-3 py-0 grey lighten-5">
      <InvoiceSelector :pos_profile="pos_profile"></InvoiceSelector>
    </v-card>
    <v-card class="cards mb-0 mt-3 py-0 grey lighten-5">
      <v-row no-gutters>
        <v-col cols="7">
        ...
        <v-col cols="5">
          <v-row no-gutters class="pa-1 pt-2 pl-0">
            ...
            <v-col class="pa-1">
              <v-btn block class="pa-0" color="success" @click="show_payment" dark>{{ total_selected_invoices > 0 ?
                __("PAY INVOICES") : __("PAY") }}</v-btn>
            </v-col>
...
export default {
  ...
  computed: {
    ...
    subtotal() {
      ...
      return this.flt(sum, this.currency_precision) + this.flt(this.total_selected_invoices);
    },
  ...
  methods: {
    ...
    show_payment() {
      ...
      if (!this.items.length && this.selected_invoices.length == 0) {
        evntBus.$emit("show_mesage", {
          text: __(`There is no Items OR no selected invoice!`),
          color: "error",
        });
        return;
      }
      ...
      if (this.get_invoice_doc().items.length) {
        const invoice_doc = this.proces_invoice();
        evntBus.$emit("send_invoice_doc_payment", invoice_doc);
      }
      else {
        evntBus.$emit("empyty_invoice");
      }
    },
